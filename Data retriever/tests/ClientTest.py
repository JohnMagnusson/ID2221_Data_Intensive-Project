...
class MyTestCase(unittest.TestCase):
    ...
    def test_getHistoricalDataWithTimeStamp(self):
        ...
        data = client.getHistoricalData(timePrefix="hour", cryptoCurrency="BTC", fiatCurrency="USD",
                                        toTimeStamp=timeStamp, nrRecordsToGet=nrRecordsToGet)
        ...
        isInTimeScope = data[u'Data'][
                            u'TimeTo'] <= timeStamp  # We check that the latest retreived timeStamp is inside the time scope have given
        self.assertTrue(isInTimeScope)

    def test_getHistoricalDataWithoutTimeStamp(self):
        nrRecordsToGet = 3
        data = client.getHistoricalData(timePrefix="hour", cryptoCurrency="BTC", fiatCurrency="USD",
                                        nrRecordsToGet=nrRecordsToGet)
        self.assertFalse(data[u'HasWarning'])
        self.assertEqual(data[u'Type'], 100)
        self.assertEqual(data[u'Response'], u'Success')
        self.assertEqual(data[u'Response'], u'Success')
    ...
    def test_assertOrderInGetHistoricalData(self):
        nrRecordsToGet = 2
        raw_data = client.getHistoricalData(timePrefix="hour", cryptoCurrency="BTC", fiatCurrency="USD",
                                            nrRecordsToGet=nrRecordsToGet)
        data = raw_data['Data']['Data']
        keys_order = ['time', 'high', 'low', 'open', 'volumefrom', 'volumeto', 'close', 'conversionType',
                      'conversionSymbol']
    ...
    def test_getHistoricalSocialDataWithoutTimeStamp(self):
        ...
        self.assertEqual(nrRecordsToGet, len(data['Data']))

    def test_getHistoricalSocialDataWithTimeStamp(self):
        nrRecordsToGet = 3
        timeStamp = 1543496126.0
        data = client.getHistoricalSocialData(timePrefix="day", cryptoCurrency="BTC", toTimeStamp=timeStamp,
                                              nrRecordsToGet=nrRecordsToGet)
        self.assertEqual(nrRecordsToGet, len(data['Data']))

    def test_assertOrderInGetHistoricalSocialData(self):
        nrRecordsToGet = 2
        raw_data = client.getHistoricalSocialData(timePrefix="hour", cryptoCurrency="BTC",
                                                  nrRecordsToGet=nrRecordsToGet, toTimeStamp=1637880400)
        data = raw_data['Data']
        keys_order = ['time', 'comments', 'posts', 'followers', 'points', 'overview_page_views', 'analysis_page_views',
                      ...
                      'twitter_following', 'twitter_lists', 'twitter_favourites', 'twitter_statuses',
                      'reddit_subscribers', 'reddit_active_users', 'reddit_posts_per_hour', 'reddit_posts_per_day',
                      'reddit_comments_per_hour', 'reddit_comments_per_day', 'code_repo_stars', 'code_repo_forks',
                      'code_repo_subscribers', 'code_repo_open_pull_issues', 'code_repo_closed_pull_issues',
